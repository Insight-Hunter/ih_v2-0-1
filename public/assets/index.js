...
export default {
    async fetch(request, env) {
        ...
        if (path === '/api/auth/login' && method === 'POST') {
            ...
            const user = await env.IH_DB.prepare('SELECT * FROM users WHERE email = ?')
                .bind(data.email)
                .first();
            if (!user)
        ...
        if (path === '/api/transactions' && method === 'POST') {
            ...
            await env.IH_DB.prepare('INSERT INTO transactions (user_id, date, description, category, amount, type) VALUES (?, ?, ?, ?, ?, ?)').bind(decoded.id, data.date, data.description || null, data.category || null, data.amount, data.type).run();
